...
export default function QuotationDetail() {
  ...
  // Simulation handlers for reviewing and testing states
  const handleSimulateApprove = async () => {
    try {
      setActionLoading(true);
      const res = await api.put(`/quotations/${id}/manager-approve`);
      showToast(res.data.message || 'Sales Manager approved!', 'success');
      await fetchQuote();
    } catch (err) {
  ...
  const handleSimulateCounter = async () => {
    try {
      setActionLoading(true);
      const res = await api.put(`/quotations/${id}/manager-counter`, {
        counter_discount: 14,
        counter_notes: 'Sales Manager counter-offer: 14% bulk discount approved for immediate quarterly execution.',
      });
      showToast(res.data.message || 'Sales Manager counter offer created!', 'success');
      await fetchQuote();
    } catch (err) {
      showToast('Simulation failed', 'error');
    } finally {
  ...
  const handleSimulateReject = async () => {
    try {
      setActionLoading(true);
      const res = await api.put(`/quotations/${id}/manager-reject`);
      showToast(res.data.message || 'Sales Manager rejected discount request', 'warning');
      await fetchQuote();
    } catch (err) {
      showToast('Simulation failed', 'error');
    } finally {
    ...
    <div className="container quotation-detail" style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '24px', margin: '24px 0' }}>
        ...
        <div style={{
          ...
          {/* Demo button to approve initial quotation if pending review */}
          {isPendingInitialApproval && (
            <button
              className="btn btn-secondary btn-sm"
              onClick={handleSimulateApprove}
              disabled={actionLoading}
              title="Simulate manager reviewing and approving the quotation proposal"
              style={{ borderColor: 'var(--warning)', color: 'var(--warning)', whiteSpace: 'nowrap' }}
            >
              {actionLoading ? 'Approving...' : '👔 [Demo] Approve as Sales Manager'}
            </button>
          )}
        </div>
      </div>
      ...
      {hasDiscountRequest && (
        <div className="discount-request-card" style={{
          ...
          <div className="discount-request-message-box">
            {/* Outcome 1: Pending Manager Review */}
            {quote.discount_request_status === 'pending_approval' && (
              <>
                <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', flex: 1 }}>
                  "Your requested discount is currently being reviewed by the Sales Manager."
                </div>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  <button
                    ...
                    onClick={handleSimulateApprove}
                    disabled={actionLoading}
                    style={{ borderColor: 'var(--success)', color: 'var(--success)', fontSize: '0.78rem' }}
                  >
                    👔 [Demo] Approve {quote.requested_discount || counterDiscount}%
                  </button>
                  <button
                    className="btn btn-secondary btn-sm"
                    onClick={handleSimulateCounter}
                    disabled={actionLoading}
                    style={{ borderColor: 'var(--info)', color: 'var(--info)', fontSize: '0.78rem' }}
                  >
                    👔 [Demo] Counter (14%)
                  </button>
                  <button
                    ...
                    disabled={actionLoading}
                    style={{ borderColor: 'var(--error)', color: 'var(--error)', fontSize: '0.78rem' }}
                  >
                    👔 [Demo] Reject
                  </button>
                </div>
      ...
      <div className="quotation-actions" style={{ justifyContent: 'space-between', alignItems: 'center', marginTop: '28px' }}>
        ...
        {(isManagerApproved || isInNegotiation) && !isConfirmed && (
          <button
            ...
            onClick={handleAcceptProposal}
            disabled={accepting}
            style={{ background: 'var(--accent)', color: '#000000', fontWeight: 800 }}
          >
