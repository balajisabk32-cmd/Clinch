...
export default function OrderTracking() {
  ...
  const handleManagerApprove = async () => {
    try {
      setApproving(true);
      await api.put(`/orders/${id}/manager-approve`);
      showToast('Sales Manager approval confirmed! Order moved to processing.', 'success');
      await fetchOrder();
    } catch (err) {
      ...
      {order.status === 'pending_approval' && (
        <div style={{
          ...
          <button
            ...
            disabled={approving}
            title="Simulate manager reviewing and approving this purchase order"
            style={{ borderColor: 'var(--warning)', color: 'var(--warning)', whiteSpace: 'nowrap' }}
          >
            {approving ? 'Approving...' : '👔 [Demo] Approve as Sales Manager'}
          </button>
        </div>
      )}
